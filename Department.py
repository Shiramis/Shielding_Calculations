...
class ddepartment():
    ...
    def creatdep(self):
        ...
        self.depcanv.configure(yscrollcommand=self.scrolldep.set,bg="#d5e0d7")
        # =========Department tab===========================
    ...
    def desroom(self, t):
        if self.d["vselroom "+str(t)].get()!="Shield room":
            if self.d["run "+str(t)] == False:
                ...
                self.roomcanv.configure(yscrollcommand=self.yscrollroom.set, xscrollcommand=self.xscrollroom.set, bg="#d5e0d7")
                ...
                self.rescanv.configure(yscrollcommand=self.scrollres.set, xscrollcommand=self.xscrollres.set, bg="#d5e0d7")
    ...
    def exp_dep(self,t):
        ...
        for b in range (1,t+1):
            ...
            for a in range(1,self.d["vnumwall " + str(b)].get()+1):
                    self.wa[self.barn["lab_bar "+str(a) + p].cget("text")]=[str(self.xlmat["thic "+str(a) + str(1) + p]),str(self.xlmat["thic "+str(a) + str(2) + p]),str(self.xlmat["thic "+str(a) + str(3) + p]),str(self.xlmat["thic "+str(a) + str(4) + p]),str(self.xlmat["thic "+str(a) + str(5) + p]),str(self.xlmat["thic "+str(a) + str(6) + p])]
            self.d["room_data {0}".format(str(b))]=pd.DataFrame(data=self.wa,index=["Lead (mm)","Concrete (mm)","Gypsum Wallboard (mm)","Steel (mm)","Plate Glass (mm)","Wood (mm)"])
